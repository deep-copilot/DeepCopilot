...
//
// Self-contained minimal unified-diff parser + applicator.
// No npm dependency. Supports:
//   - Multiple files per patch (--- / +++ headers)
//   - Multiple hunks per file (@@ headers)
//   - ±3-line fuzz (like GNU patch) for whitespace-drifted context
//   - CRLF / LF normalization
//   - Atomic per-file write (all hunks must succeed before touching disk)
//   - Workspace boundary enforcement via ensurePathAllowed

function _normalizeLines(text) {
    // Normalize to LF for internal processing, remember original endings
...
function _restoreEndings(lines, hasCRLF) {
    ...
    if (hasCRLF) return joined.replace(/\n/g, '\r\n');
    return joined;
}

// Parse a unified diff into [{oldPath, newPath, hunks:[{oldStart,oldCount,newStart,newCount,lines:[]}]}]
function _parsePatch(patchText) {
    ...
    for (const line of rawLines) {
        if (line.startsWith('--- ')) {
            ...
            // Context lines may be '' in some generators (treated as ' ')
            const prefix = line.length === 0 ? ' ' : line[0];
            const content = line.length === 0 ? '' : line.slice(1);
            if (prefix === '+' || prefix === '-' || prefix === ' ') {
                curHunk.lines.push({ op: prefix, text: content });
            }
        }
    }
    return files;
...
// Try to apply one hunk to `lines` (0-indexed array, LF-normalized).
// Returns { ok: true, lines: newLines } or { ok: false, reason: string }.
function _applyHunk(lines, hunk, fuzz = 0) {
    // Context lines from the hunk (op === ' ' or '-')
    const contextAndRem = hunk.lines.filter(l => l.op === ' ' || l.op === '-').map(l => l.text);
    if (contextAndRem.length === 0 && hunk.oldCount === 0) {
        // Pure insertion — insert at hunk.oldStart
        ...
        const result = [...lines.slice(0, insertAt), ...additions, ...lines.slice(insertAt)];
        return { ok: true, lines: result };
    }

    // Find anchor: search for the first context/removal line (stripped if fuzz > 0)
    const anchorLine = hunk.oldStart - 1; // preferred position (0-indexed)

    // Search window: prefer near anchorLine, expand by fuzz
    ...
    for (let start = searchStart; start <= searchEnd; start++) {
        ...
        for (let i = 0; i < contextAndRem.length; i++) {
            ...
            if (actual === expected) {
                matches++;
            } else if (fuzz > 0 && actual.trim() === expected.trim()) {
                matches += 0.8;
            }
        }
    ...
    if (bestScore < threshold) {
        return {
            ...
            reason: `Could not find context at ~line ${hunk.oldStart} (best match score ${(bestScore * 100).toFixed(0)}%). ` +
                ...
                `Actual lines ${hunk.oldStart - 1}–${hunk.oldStart + 2}: ${lines.slice(Math.max(0, hunk.oldStart - 1), hunk.oldStart + 3).map(l => JSON.stringify(l)).join(', ')}`,
        };
    }

    // Apply: walk through hunk.lines from bestMatch
    let pos = bestMatch;
...
async function toolApplyPatch(args) {
    ...
    for (const fileDiff of fileDiffs) {
        // Resolve path: prefer newPath (could be /dev/null for deletions)
        const relPath = (fileDiff.newPath && fileDiff.newPath !== '/dev/null')
            ? fileDiff.newPath
            : fileDiff.oldPath;

        const absPath = resolvePath(relPath);
        const allowed = await ensurePathAllowed(absPath, 'write');
        if (!allowed) {
        ...
        // Read original file (or start with empty for new files)
        let originalText = '';
        let hasCRLF = false;
        ...
        // Remove trailing empty line that split() may add
        ...
        if (!ok) {
            report.push(`❌ ${relPath}: ${hunkReports.filter(h => h.includes('✓') === false || h.includes('Hunk')).join('\n')}`);
        ...
        try {
            ...
            report.push(`❌ ${relPath}: write error — ${e.message}`);
            anyFailed = true;
        }
    }

    const summary = anyFailed
        ? `apply_patch: ${fileDiffs.length - report.filter(r => r.startsWith('✓')).length} file(s) failed. See details:\n`
